...
UInt32 os_getmaxprintfcharsinK()
{
    OSMutexLocker locker(OS::GetStdLibMutex());
    return sMaxTotalCharsInK;
}

void os_setmaxprintfcharsinK(UInt32 newMaxCharsInK)
{
    OSMutexLocker locker(OS::GetStdLibMutex());
    sMaxTotalCharsInK = newMaxCharsInK;
}

int os_maxprintf(const char *fmt,  ...)
{
    if (fmt == NULL)
        return -1;
...
int os_fprintf(FILE *file, const char *fmt,  ...)
{
    if (file == NULL)
        return -1;
...
size_t os_strftime(char *buf, size_t maxsize, const char *format, const struct tm *timeptr)
{
    if (buf == NULL)
        return 0;
...
char *os_strerror(int errnum, char* buffer, int buffLen)
{
    OSMutexLocker locker(OS::GetStdLibMutex());
...
char *os_ctime(const time_t *timep, char* buffer, int buffLen)
{
#if __MacOSX__
    Assert(buffLen >= 26);
...
char *os_asctime(const struct tm *timeptr, char* buffer, int buffLen)
{
#if __MacOSX__
    Assert(buffLen >= 26);
...
struct tm *os_gmtime(const time_t *timep, struct tm *result)
{
#if __MacOSX__
...
struct tm *os_localtime(const time_t *timep, struct tm *result)
{
 #if __MacOSX__
