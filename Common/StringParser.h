...
inline std::string & rtrim(std::string & str, const std::string & ws)
{
...
inline std::vector<std::string>::size_type split(const std::string & str, char delim, std::vector<std::string>& elems)
{
...
inline bool get_ip_port_from_str(std::string &ip,int &port,const std::string& strAddr)
{
	int idx = strAddr.find(':');
	if(idx != -1)
	{
		ip = strAddr.substr(0,idx);
		port = atoi(strAddr.substr(idx + 1,strAddr.size() - idx - 1).c_str());
		return true;
	}
	return false;
}
...
template <typename T>
T value_from_string(const std::string & data)
{
    T value;
    std::istringstream iss(data);
    iss >> value;
    if (iss.fail()) 
        LOG_ERROR("Convert From String To Other Type Value Falied:%s",data.c_str());
    return value;
}
