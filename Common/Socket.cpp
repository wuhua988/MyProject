...
Socket::Socket(UInt32 inSocketType)
...
int Socket::GetSocketError()
{
    ...
    if (::getsockopt(fFileDesc, SOL_SOCKET, SO_ERROR, &optval, &optlen) < 0)
    {
...
{   
    InitNonBlocking(fFileDesc);
}

void Socket::InitNonBlocking(int fd)
{
    #ifdef __Win32__
    u_long one = 1;
